...
bool nextTagCharmPerkeo(Instance inst, int ante) {
...
std::unique_ptr<SearchFilter> createFilter() {
    auto filterFunc = [](const std::string& seed) -> int {
        ...
        auto firstTag1 = nextTagCharmPerkeo(inst, 1);
        auto firstTag2 = nextTagCharmPerkeo(inst, 2);

        if(!firstTag1 && !firstTag2) return 0;
    ...
    return createCustomFilter(filterFunc, {"Perkeo", "Telescope", "Observatory"}, "Observatory on Ante 2 + Perkeo and Temp in Skip Tag of small blind");
}